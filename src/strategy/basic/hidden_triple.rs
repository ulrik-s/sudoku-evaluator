...
use crate::board::{Board, Unit, CandidateCoords};
use crate::SolverError;

pub struct HiddenTriple;

impl Strategy for HiddenTriple {
    fn kind(&self) -> StrategyKind { StrategyKind::HiddenTriple }
...
fn search_unit(board: &mut Board, unit: Unit) -> Result<bool, SolverError> {
    ...
    for d in 1..=9 { positions[d as usize] = board.candidate_coords(unit, d); }
    for d1 in 1..=7 {
        for d2 in d1+1..=8 {
            for d3 in d2+1..=9 {
                let mut union: Vec<(usize,usize)> = Vec::new();
                for pos in positions[d1 as usize].iter() { if !union.contains(&pos) { union.push(pos); } }
                for pos in positions[d2 as usize].iter() { if !union.contains(&pos) { union.push(pos); } }
                for pos in positions[d3 as usize].iter() { if !union.contains(&pos) { union.push(pos); } }
                if union.len() == 3 &&
