...
impl Strategy for HiddenQuad {
    fn kind(&self) -> StrategyKind { StrategyKind::HiddenQuad }

    fn apply(&self, board: &mut Board) -> Result<bool, SolverError> {
        for r in 0..9 { if search_row(board, r)? { return Ok(true); } }
        for c in 0..9 { if search_col(board, c)? { return Ok(true); } }
        for br in 0..3 {
            for bc in 0..3 {
                if search_box(board, br * 3, bc * 3)? { return Ok(true); }
            }
        }
        Ok(false)
    }
...
fn search_row(board: &mut Board, r: usize) -> Result<bool, SolverError> {
    ...
    for c in 0..9 {
        if board.get(r, c).is_none() {
            for d in board.candidates(r, c) { positions[d as usize].push(c); }
        }
    }
...
fn search_col(board: &mut Board, c: usize) -> Result<bool, SolverError> {
    ...
    for r in 0..9 {
        if board.get(r, c).is_none() {
            for d in board.candidates(r, c) { positions[d as usize].push(r); }
        }
    }
...
fn search_box(board: &mut Board, start_r: usize, start_c: usize) -> Result<bool, SolverError> {
    ...
    for r in start_r..start_r + 3 {
        for c in start_c..start_c + 3 {
            if board.get(r, c).is_none() {
                for d in board.candidates(r, c) { positions[d as usize].push((r,c)); }
            }
        }
    }
    for d1 in 1..=6 {
        for d2 in d1+1..=7 {
            for d3 in d2+1..=8 {
                for d4 in d3+1..=9 {
                    let union: Vec<(usize,usize)> = positions[d1 as usize].iter()
...
fn apply_hidden<F>(positions: [Vec<usize>;10], idx_to_coord:F, board:&mut Board) -> Result<bool,SolverError>
        ...
        for d2 in d1+1..=7 {
            for d3 in d2+1..=8 {
                for d4 in d3+1..=9 {
                    ...
                    for &p in pos2 { if !union.contains(&p) { union.push(p); } }
                    for &p in pos3 { if !union.contains(&p) { union.push(p); } }
                    for &p in pos4 { if !union.contains(&p) { union.push(p); } }
                    if union.len()==4 {
                        let mut changed=false;
                        for &idx in &union {
