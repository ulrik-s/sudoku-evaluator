...
impl Strategy for YWing {
    fn kind(&self) -> StrategyKind { StrategyKind::YWing }
    ...
    fn apply(&self, board: &mut Board) -> Result<bool, SolverError> {
        if crate::board::row_indices()
            ...
            .find_map(|(r, c)| {
                ...
                if pivot.len() != PAIR_LEN { return None; }
                ...
                let first_wings: Vec<_> = peers
                    ...
                    .filter_map(|(r1, c1)| {
                        ...
                        cand1.iter().find(|&d| d != d1).map(|other| ((r1, c1), other))
                    })
                    .collect();

                first_wings
                    .into_iter()
                    .find_map(|((r1, c1), other)| {
                        ...
                        second_wings
                            ...
                            .find_map(|(r2, c2)| {
                                ...
                                peers1
                                    ...
                                    .try_fold(false, |changed, &(rr, cc)| match board.eliminate_candidate(rr, cc, other) {
                                        Some(true) => Ok(true),
                                        Some(false) => Ok(changed),
                                        None => Err(SolverError::Contradiction { row: rr, col: cc }),
                                    })
                                    .map_or_else(|e| Some(Err(e)), |changed| if changed { Some(Ok(true)) } else { None })
                            })
                    })
            })
            .transpose()?
